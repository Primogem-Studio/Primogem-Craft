		...
		if (!world.isClientSide()) {
			...
			if (entity.getRemainingFireTicks() > 0) {
				if (!(sourceentity instanceof Player _plrCldCheck3 && _plrCldCheck3.getCooldowns().isOnCooldown(itemstack.getItem()))) {
					...
					entity.hurt(
							FullToElementDamageProcedure.execute(new DamageSource(world.holderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("primogemcraft:s_hchixushanghai"))), sourceentity), true, false, true, false, 1,
									1),
